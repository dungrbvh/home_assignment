...
Check whether monitoring is measuring the root filesystem, a mount or innodes. 
...
# OR compress recent logs
sudo gzip -c /var/log/nginx/access.log > /tmp/access.log.$(date +%F_%H%M).gz   # only if /tmp has space
sudo gzip -c /var/log/nginx/access.log > /tmp/access.log.$(date +%F_%H%M).gz   # only if /tmp has space
